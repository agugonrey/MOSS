...
dbscan_BHI <- function(data,eps=NULL,minPts=5,prop_outliers=.1,eps_res=500,eps_range=NULL,annot,plot=F){
  ...
  if(is.null(eps)){
    ...
    if (plot) graphics::plot(x=epsvec,y=seq(0,1,length.out = length(epsvec)),pch=NA,xlab="EPS",ylab="BHI")
    for(i in 1:length(epsvec)){
      eps<-epsvec[i]
      DBcl<-dbscan::dbscan(data,eps,MinPts = minPts) # quite fast
      cl<-DBcl$cluster
      cat(".")
      if(all(cl==1)) break else if(max(cl)==1) bhi[i] <-0 else
        if(all(cl==0))  bhi[i] <-0 else
          if(mean(cl==0)>prop_outliers) bhi[i] <-0 else{
            bhi[i]   <- BHI(statClust = cl[cl!=0],annotation = annot[cl!=0,]) # exclude the 0'
          }
  ...
  if(!is.null(eps)) bhi <- BHI(statClust = DBcl[DBcl!=0],annotation = annot[DBcl!=0,]) # exclude the 0'
