...
rrr_pca <- function(data.blocks, block.class,K.X,K.Y,ncores,verbose,M) {
  ...
  #If any data block is of class "FBM", the extended data matrix is turned into an "FBM".
  if (any(block.class == "FBM")) {
    ...
    for (m in 1 : M) {
      ...
      block.chunks <- rep(1 : ceiling(p[m] / bigstatsr::block_size(p[m],1)), each = bigstatsr::block_size(p[m],1))[1 : p[m]]
      n.chunks <- length(unique(block.chunks))
      for (chunk in unique(block.chunks)) {
        if (verbose) cat("   Working chunk ", chunk, " of ",n.chunks,"\n")
        Z[, ext.mat.slides[b.names == names(data.blocks)[m]][block.chunks == chunk]] <- data.blocks[[m]][,block.chunks == chunk]
      }
    }
  }
  #If all blocks are of class matrix, simply appends data blocks by column.
  else Z <- do.call("cbind", data.blocks)
...
rrr_mb_pca <- function(data.blocks,block.class,K.X,K.Y,ncores,verbose,M) {
  ...
  if (M == 1) return(rrr_pca(data.blocks))
  ...
  for (m in 2 : (M+1)) {
    ...
    SVD[[m]] <- rrr_big(X = data.blocks[[m - 1]],K.X = K.X,verbose = verbose,ncores = ncores)
  ...
  if (inherits(Z, "FBM") == TRUE) {
    ...
    SVD[[1]]$u <-
      bigstatsr::big_apply(Z, function(X,ind) {
        if (verbose) cat(" Working chunk ",(count <<- count + 1),"of",ceiling(ncol(Z) / bigstatsr::block_size(ncol(Z), 1)),".\n")
        Z[,ind] %*% SVD[[1]]$v[ind,]
      },ind=seq_len(ncol(Z)),
      a.combine = 'plus',
      block.size = bigstatsr::block_size(ncol(Z), 1))
  }
  else SVD[[1]]$u <- Z %*% SVD[[1]]$v
  #Rotating u.
...
rrr_pls <- function(data.blocks,block.class,K.X,K.Y,ncores,verbose,M) {
  ...
  #If any data block is of class "FBM", the extended predictors matrix is turned into an "FBM".
  if (any(block.class == "FBM")) {
    #Creating an extended omic matrix.
    if (verbose) message("Creating an extended data matrix.")
  ...
  #If all blocks are of class matrix, simply appends data blocks by column.
  else Z <- do.call("cbind", data.blocks[-1])
  s <- rrr_big(X = Z, Y=data.blocks[[1]], K.X = K.X, K.Y=K.Y, verbose = verbose,ncores = ncores)
  return(s)
}
...
rrr_mb_pls <- function(data.blocks,block.class,K.X,K.Y,ncores,verbose,M) {
  ...
  if (any(block.class == "FBM")) {
    ...
    for (m in 2 : M) {
      ...
      block.chunks <- rep(1 : ceiling(p[m] / bigstatsr::block_size(p[m],1)),
                          each = bigstatsr::block_size(p[m],1))[1 : p[m]]
      n.chunks <- length(unique(block.chunks))
      for (chunk in unique(block.chunks)) {
        if (verbose) cat("   Working chunk ", chunk, " of ",n.chunks,"\n")
        Z[, ext.mat.slides[b.names == names(data.blocks)[m]][block.chunks == chunk]] <-
  ...
  for (m in 2 : M) {
    ...
    SVD[[m]] <- rrr_big(Y=data.blocks[[1]],
                        X = data.blocks[[m]],
                        K.X = K.X, K.Y = K.Y,
                        verbose = verbose,ncores = ncores)$SVD
    SVD[[1]]$d <- SVD[[1]]$d + SVD[[m]]$d
  ...
  LR <- rrr_big(Y = Z, X=data.blocks[[1]],
                K.X = K.Y, K.Y=K.X,
                verbose = verbose,
                ncores = ncores,
                lr.return = T)
  ...
  if (inherits(LR, "FBM") == TRUE) {
    ...
    SVD[[1]]$u <-
      bigstatsr::big_apply(LR, function(X,ind) {
        if (verbose) cat("Working chunk ",
                         (count <<- count + 1),"of",
                         ceiling(ncol(LR) / bigstatsr::block_size(ncol(LR), 1)),".\n")
        LR[,ind] %*% SVD[[1]]$v[ind,]
      }, ind=seq_len(ncol(LR)),
      a.combine = 'plus',
      block.size = bigstatsr::block_size(ncol(LR), 1))
  }
  else SVD[[1]]$u <- LR %*% SVD[[1]]$v
  #Rotating u.
  if (verbose) message("Generating orthogonal axes.")
  SVD[[1]]$u <- qr.Q(qr(SVD[[1]]$u))
  aux <- SVD[[1]]
...
rrr_rrr <- function(data.blocks,block.class,K.X,K.Y,ncores,verbose,M) {
  ...
  s <- rrr_big(X = Z, Y=data.blocks[[1]],power=-1, K.X = K.X, K.Y=K.Y, verbose = verbose,ncores = ncores)
  return(s)
}
...
rrr_lda <- function(data.blocks,block.class,K.X,ncores,verbose,M) {
  ...
  data.blocks[[1]] <- apply(data.blocks[[1]], 2, function(x) {
    ...
    aux <- as.numeric(as.character(factor(x,
                                          ...
                                          labels = c(-n.x.elements[1], n.x.elements[2])/sum(n.x.elements))))
    return(aux)
  })
...
rrr_mb_lda <- function(data.blocks,block.class,K.X,ncores,verbose,M) {
  ...
  for (m in 2 : M) {
    ...
    SVD[[m]] <- rrr_lda(data.blocks=data.blocks[c(1,m)],
                        block.class = block.class[c(1,m)],
                        K.X = K.X,
                        verbose = verbose,ncores = ncores,M=2)$SVD
    SVD[[1]]$d <- SVD[[1]]$d + SVD[[m]]$d
